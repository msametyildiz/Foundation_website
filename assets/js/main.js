// Ultra Modern Main JavaScript - Enhanced for Performance, A11y, and UX
document.addEventListener('DOMContentLoaded', function() {
    
    // --- Performance & Accessibility Initialization ---
    initializePerformanceObserver();
    initializeAdvancedAnimations();
    initializeParallaxEffects(); // Will be conditional inside the function
    ...
    // --- Smooth Scroll for Anchor Links (Navbar Offset) ---
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (target) {
                const navbarHeight = document.querySelector('.navbar-modern')?.offsetHeight || 85;
    ...
    // --- Performance: Adapt to user's network and device capabilities ---
    function initializePerformanceObserver() {
        ...
        if (navigator.connection && 
           (navigator.connection.saveData || 
            ...
            console.log('Low performance mode activated due to slow network.');
        }
        ...
        if (motionQuery.matches) {
            document.body.classList.add('low-performance-mode');
            console.log('Low performance mode activated due to prefers-reduced-motion.');
        }
    }

    // --- Parallax Effects (Conditional Initialization) ---
    function initializeParallaxEffects() {
        // Only run parallax on devices that can handle it
        if (document.body.classList.contains('low-performance-mode') || window.innerWidth < 1024) {
    ...
    // --- Advanced Animations (Conditional Initialization) ---
    function initializeAdvancedAnimations() {
        if (document.body.classList.contains('low-performance-mode')) {
            return; // Skip animations
        }

        const animatedElements = document.querySelectorAll('.animate-fadeInUp, .floating-card, .project-card');
