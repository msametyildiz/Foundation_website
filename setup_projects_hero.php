...
try {
    // Hero ayarları tablosunu oluştur
    $sql = "CREATE TABLE IF NOT EXISTS `projects_hero_settings` (
    ...
    $pdo->exec($sql);
    echo "✓ projects_hero_settings tablosu oluşturuldu.<br>";
    
    // Varsayılan ayarları ekle
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM projects_hero_settings WHERE is_active = 1");
    $stmt->execute();
    $count = $stmt->fetchColumn();
    
    if ($count == 0) {
        $insertSql = "INSERT INTO `projects_hero_settings` 
        ...
        $pdo->exec($insertSql);
        echo "✓ Varsayılan hero ayarları eklendi.<br>";
    } else {
        echo "✓ Hero ayarları zaten mevcut.<br>";
    }
    
    echo "<br><strong>Kurulum başarıyla tamamlandı!</strong><br>";
    echo "<a href='index.php'>Ana sayfaya git</a> | <a href='admin/'>Admin panele git</a>";
